...
def datasets() -> dict[str, list[Point]]:
    """Read common/data.json fresh — sync regenerates it in the same process, just before these run."""
    return {name: [(x, y) for x, y in points] for name, points in json.loads((QUESTION / "common" / "data.json").read_text()).items()}
...
def write_solutions(path: pathlib.Path) -> None:
    """Emit solutions.json — the expected kmeans() output per dataset.

    Data, not code: it was a .py literal until the generated files moved out of the source
    tree, where an importable module would have had to be excluded from every checker.
    Clusters are sorted by centroid and points within a cluster are sorted, so the file is
    stable across regenerations — ordering is not part of the contract.
    """
    answers = {
        name: [[list(centroid), [list(p) for p in sorted(pts)]] for centroid, pts in sorted(solve(points, K))]
        for name, points in datasets().items()
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({"K": K, "ANSWERS": answers}, indent=2) + "\n")


def write_answers(path: pathlib.Path) -> None:
    """Emit answers.md — the reference answers, as markdown."""
    sets = datasets()
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w") as fh, contextlib.redirect_stdout(fh):
        print("# Reference answers\n")
        print("Generated by `answers.py` — best of", N_INIT, "k-means++ restarts, seed", SEED)
        print("\n**Answer key — do not share with candidates.**\n")

        print("## TWENTY — full expected output\n")
        print("```text")
        print_clusters(solve(sets["TWENTY"], 3))
        print("```\n")
        ...
        print("```text")
        elongated_answer = solve(sets["ELONGATED"], 3)
        show(
        ...
        for k in (2, 3, 5, 8):
            _summary("UNIFORM", sets["UNIFORM"], k)


if __name__ == "__main__":
    write_solutions(BUILD / "solutions.json")
    write_answers(BUILD / "answers.md")
    print("wrote solutions.json, answers.md")
