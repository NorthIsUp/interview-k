...
def write_solutions(path: pathlib.Path) -> None:
    """Emit solutions.py — the expected clustering per dataset, as checked-in data."""
    lines = [
    ...
    for name, points in _ALL:
        clusters = solve(points, K)
        inertias[name] = _inertia(clusters)
        sizes[name] = [len(p) for _, p in sorted(clusters)]
        lines.append(f"    {name!r}: [")
        for centroid, _ in sorted(clusters):
            lines.append(f"        ({centroid[0]!r}, {centroid[1]!r}),")
        lines.append("    ],")
    lines += ["}", "", "SIZES: dict[str, list[int]] = {"]
    lines += [f"    {n!r}: {s!r}," for n, s in sizes.items()]
    lines += ["}", "", "INERTIA: dict[str, float] = {"]
    lines += [f"    {n!r}: {v!r}," for n, v in inertias.items()]
    lines += ["}", ""]
    path.write_text("\n".join(lines))
